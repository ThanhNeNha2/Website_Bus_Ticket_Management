...
const AddInfoPromotion = ({
  ...
  const onSubmit = (e) => {
    ...
    handleAddPromotion(e); // Call the submit handler if valid
  };
  ...
  const handleTripSelection = (e) => {
    ...
    if (selectedTripId && !formData.applicableTrips.includes(selectedTripId)) {
      handleInputChange({
        target: {
          name: "applicableTrips",
          value: [...formData.applicableTrips, selectedTripId],
        },
      });
    }
  ...
  const handleRemoveTrip = (tripId) => {
    handleInputChange({
      target: {
        name: "applicableTrips",
        value: formData.applicableTrips.filter((id) => id !== tripId),
      },
    });
  };
  ...
  const availableTrips = trips.filter(
    (trip) => !formData.applicableTrips.includes(trip._id)
  );
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center mt-14">
      <div className="bg-white p-6 rounded-lg w-[800px]">
        <h3 className="text-xl font-bold mb-4">Thêm mã giảm giá mới</h3>
        <form onSubmit={onSubmit}>
            ...
            <div className="mb-4 flex-1">
              ...
              <div className="w-full border px-3 py-2 rounded h-24 overflow-y-auto">
                {formData.applicableTrips.length > 0 ? (
                  formData.applicableTrips.map((tripId) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => handleRemoveTrip(tripId)}
          ...
          {addPromotionMutation.isError && (
            <p className="text-red-500 text-sm mb-4">
              {addPromotionMutation.error.message}
            </p>
          )}
          <div className="flex justify-end">
            ...
            <button
              ...
              disabled={addPromotionMutation.isLoading || discountError !== ""}
            >
              {addPromotionMutation.isLoading ? "Đang xử lý..." : "Thêm"}
            </button>
          </div>
        </form>
      </div>
...
export default AddInfoPromotion;
